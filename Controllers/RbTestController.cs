...
namespace ExpressBase.Web.Controllers
{
    ...
    public class RbTestController : EbBaseNewController
    {
        ...
        public IActionResult Index()
        {
            ...
            {
                rec = new iTextSharp.text.Rectangle(repdef.PaperSize.Height, repdef.PaperSize.Width);
            }
            else
            {
                rec = new iTextSharp.text.Rectangle(repdef.PaperSize.Width, repdef.PaperSize.Height);
            }
            ...
            foreach (EbReportField c in s.Fields)
            {
                foreach (var col in cresp.Columns)
                {
                    if (col.ColumnName == c.Name)
                    {
                        ...
                        ColumnText ct = new ColumnText(cb);
                        ct.SetSimpleColumn(new Phrase(gtotval), c.Left, repdef.PaperSize.Height - (c.Top + c.Height), c.Left + c.Width, c.Top - 10, 15, Element.ALIGN_LEFT);
                        ct.Go();
                    }
                }
            }
        ...
        public void repfldInitialize()
        {
           ...
           // cntrl6.DecimalPlaces = 4;
            ...
            {
                s.Height = 50;
                s.Fields = new List<EbReportField>();
