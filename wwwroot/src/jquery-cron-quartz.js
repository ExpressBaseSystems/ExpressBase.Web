...
; (function ($) {
    ...
    $.cronBuilder = function (el, options) {
        ...
        base.init = function () {
            ...
            if (typeof base.options.onChange === "function") {
                base.$el.find("select, input").change(function () {
                    ...
                    base.options.onChange(base.CreateEbCron());
                });
            }
        ...
        base.CreateEbCron = function () {            
            EbCron.Type = $("input[name=sch-type]:checked").val();
            if (EbCron.Type === "reccur") {
                EbCron.period = base.$el.find("select.cron-period-select").val();
                EbCron.min = base.$el.find("select.cron-clock-minute").val();
                EbCron.hour = base.$el.find("select.cron-clock-hour").val();

                switch (EbCron.period) {
                    case 'Daily':
                        var $_selector = base.$el.find("div.cron-daily");

                        EbCron.periodObj = {
                            Type: $_selector.find("input[name=dailyType]:checked").val(),
                            Days: $_selector.find("select.cron-daily-select").val()
                        };
                        break;
                    case 'Weekly':
                        $_selector = base.$el.find("div.cron-weekly");
                        var ndow = [];
                        if ($_selector.find("input[name=dayOfWeekMon]").is(":checked"))
                            ndow.push("MON");
                        if ($_selector.find("input[name=dayOfWeekTue]").is(":checked"))
                            ndow.push("TUE");
                        if ($_selector.find("input[name=dayOfWeekWed]").is(":checked"))
                            ndow.push("WED");
                        if ($_selector.find("input[name=dayOfWeekThu]").is(":checked"))
                            ndow.push("THU");
                        if ($_selector.find("input[name=dayOfWeekFri]").is(":checked"))
                            ndow.push("FRI");
                        if ($_selector.find("input[name=dayOfWeekSat]").is(":checked"))
                            ndow.push("SAT");
                        if ($_selector.find("input[name=dayOfWeekSun]").is(":checked"))
                            ndow.push("SUN");
                        EbCron.periodObj = ndow;
                        break;
                    case 'Monthly':
                        $_selector = base.$el.find("div.cron-monthly");
                        if ($_selector.find("input[name=monthlyType][value=byDay]").is(":checked")) {
                            EbCron.periodObj = {
                                Type: $_selector.find("input[name=monthlyType]:checked").val(),
                                nMonth: $_selector.find("select.cron-monthly-month").val(),
                                DayOfMonth: $_selector.find("select.cron-monthly-day").val()
                            };
                        }
                        else {
                            EbCron.periodObj = {
                                Type: $_selector.find("input[name=monthlyType]:checked").val(),
                                DayOfWeek: $_selector.find("select.cron-monthly-day-of-week").val(),
                                DayOfMonth: $_selector.find("select.cron-monthly-nth-day").val(),
                                MonthByWeek: $_selector.find("select.cron-monthly-month-by-week").val()

                            };
                        }
                        break;
                    case 'Yearly':
                         $_selector = base.$el.find("div.cron-yearly");

                        if ($_selector.find("input[name=yearlyType][value=byDay]").is(":checked")) {
                            EbCron.periodObj = {
                                Type: $_selector.find("input[name=yearlyType]:checked").val(),
                                DayOfYear: $_selector.find("select.cron-yearly-day").val(),
                                MonthOfYear: $_selector.find("select.cron-yearly-month").val(),
                            };
                        } else {
                            EbCron.periodObj = {
                                Type: $_selector.find("input[name=yearlyType]:checked").val(),
                                DayOfWeek: $_selector.find("select.cron-yearly-day-of-week").val(),
                                nDay: $_selector.find("select.cron-yearly-nth-day").val(),
                                Month: $_selector.find("select.cron-yearly-month-by-week").val()
                            };
                        }
                        break;
                }
            }
            console.log("EbCron: " + JSON.stringify(EbCron));
        };
