var formBuilder = function (toolBoxid, formid) {
    ...
    this.onClickFn = function (el, target, source, sibling) {
        if ($(source).attr("id") === "form-buider-toolBox") {
            ...
            $.each($('#' + this.formid + " table [class=tdDropable]"), this.pushContainers.bind(this));
        }
        else
            console.log("else : removed");
    }
    this.drake.on("drop", this.onClickFn.bind(this));
};